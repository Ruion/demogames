...
public class PatchBuilderWindow : EditorWindow
{
    ...
    #endregion GUI Context

    #region Members

    private PatchManager m_patchManager;
    private GUIContext m_guiContext;
    ...
    private Vector2 _newVersionWindowScrollPosition = Vector2.zero;
    ...
    // Patches builder
    private Vector2 _patchesBuilderWindowScrollPosition = Vector2.zero;
    ...
    // Deploy
    private Vector2 _deployWindowScrollPosition = Vector2.zero;
    ...
    // Upload to FTP/SFTP
    private Vector2 _uploadWindowScrollPosition = Vector2.zero;
    ...
    #endregion Members for Patch Builder context
    ...
    #endregion Callbacks for New version

    #region Callbacks for Patch build

    private void OnSetMainProgressBarPatchBuild(int min, int max)
    {
        _mainProgressBarMaxPatchBuild = max;
        _mainProgressPatchBuild = 0;
    }

    private void OnSetDetailProgressBarPatchBuild(int min, int max)
    {
        _detailProgressBarMaxPatchBuild = max;
        _detailProgressPatchBuild = 0;
    }

    private void OnIncreaseMainProgressBarPatchBuild()
    {
        _mainProgressPatchBuild++;
    }

    private void OnIncreaseDetailProgressBarPatchBuild()
    {
        _detailProgressPatchBuild++;
    }

    private void OnLogPatchBuild(string main, string detail)
    {
        _mainLogPatchBuild = main;
        _detailLogPatchBuild = detail;
    }

    private void OnErrorPatchBuild(string main, string detail, Exception e)
    {
        _mainLogPatchBuild = main;
        _detailLogPatchBuild = detail;
    }

    private void OnFatalErrorPatchBuild(string main, string detail, Exception e)
    {
        _mainLogPatchBuild = main;
        _detailLogPatchBuild = detail;
    }

    private void OnTaskStartedPatchBuild(string message)
    {
        _patchBuilderStatus = PatchBuilderStatus.IS_BUILDING;
        _endLogPatchBuild = message;
    }

    private void OnTaskCompletedPatchBuild(string message)
    {
        _currentPatches = m_patchManager.GetCurrentPatches();
        UpdateLastVersionGUI(_lastVersion);
        _endLogPatchBuild = message;
    ...
    #endregion Callbacks for Patch build
